// Package cli builds the thegradient command tree.
package cli
...
import (
	...
	"github.com/tamnd/thegradient-cli/thegradient"
)
...
type App struct {
	client   *thegradient.Client
	cfg      thegradient.Config
	output   string
	fields   []string
	noHeader bool
	template string
...
func Root() *cobra.Command {
	app := &App{cfg: thegradient.DefaultConfig()}

	root := &cobra.Command{
		Use:           "thegradient",
		Short:         "Browse The Gradient AI publication from the command line",
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = thegradient.NewClient(a.cfg)
	return nil
}
