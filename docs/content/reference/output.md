...
describes what users get. Pick a format with `-o`, or let thegradient choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
thegradient <command> -o table   # aligned columns for reading
thegradient <command> -o jsonl   # one JSON object per line, for piping
thegradient <command> -o json    # a single JSON array
thegradient <command> -o csv     # spreadsheet friendly
thegradient <command> -o tsv     # tab-separated
thegradient <command> -o url     # just the URL column
thegradient <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
thegradient <command> --fields id,title,url
```
...
```bash
thegradient <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
thegradient <command>            # a table, because this is a terminal
thegradient <command> | wc -l    # JSONL, because this is a pipe
```
